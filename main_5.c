...
int main()
{
  ...
  ofstream texFile;
  texFile.open("Anharmonic_10.tex", fstream::trunc);
  ...
  texFile << "$6\\cdot\\left([-X,H_{0}] + \\frac{1}{2!}[-X,[-X,H_{0}]] + \\frac{1}{3!}[-X,[-X,[-X,H_{0}]]]\\right)$" << endl;

  texFile << "\\begin{table}[!hp]"                                                << endl;
  texFile << "\\begin{center}"                                                    << endl;
  texFile << "\\begin{tabular}{rcl}"                                              << endl;

  vector<Term> SixTermVect;
  SixTermVect.push_back(Term("(6)"));
  Polynomial Six(SixTermVect);
  CommutatorResult       *= Six;
  CommutatorResult.groupTerms();
  SecondCommutatorResult *= Six;
  SecondCommutatorResult.groupTerms();

  Polynomial PreFinalComputation  = CommutatorResult+SecondCommutatorResult+ThirdCommutatorResult;
  ...
  texFile << "$6\\cdot\\left(H_{4} - U^{\\dagger}H_{0}U\\right)$ & = & $6*\\left(\\Lambda_{4}\\right)$ \\\\" << endl;
  texFile << "                             &   & \\\\" << endl;
  texFile << "                             & = & $6\\cdot\\left(\\frac{\\lambda}{4}(A+B)^{4} - \\left([-X,H_{0}] + \\frac{1}{2!}[-X,[-X,H_{0}]] + \\frac{1}{3!}[-X,[-X,[-X,H_{0}]]]\\right)\\right)$ \\\\" << endl;
  texFile << "                             &   & \\\\" << endl;
  texFile << "                             & = & $";
